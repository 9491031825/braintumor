...
def predict():
    ...
    # Get the total area of the image in pixels (width * height)
    ...
    tumor_area = np.sum(binary_mask)  # Count pixels with value 1
    ...
    # Return paths to the images, tumor area, and total area in pixels in the JSON response
    return jsonify({
        "uploaded_image": f"static/uploaded_image_{unique_id}.png",
        "prediction": f"static/prediction_mask_{unique_id}.png",
        "overlay": f"static/overlay_image_{unique_id}.png",
